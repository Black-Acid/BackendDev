...
async def handle_conversation(user_id: str, message: str, retriever):
    ...
    # --- CASE 1: NEW CONVERSATION OR NEW SYMPTOM ---
    if session is None or intent == "new_symptom":
        # ensure retriever is ready (lazy load if necessary)
        await initialize_services()
        current_retriever = retriever  # global retriever populated by initialize_services

        # call retriever in an async-safe manner
        if hasattr(current_retriever, "ainvoke"):
            docs = await current_retriever.ainvoke(message)
        else:
            docs = await asyncio.to_thread(current_retriever.invoke, message)
...
async def get_response2(message: str):
    ...
    # use async-safe invocation depending on retriever capabilities
    if hasattr(current_retriever, "ainvoke"):
        docs = await current_retriever.ainvoke(message)
    else:
        docs = await asyncio.to_thread(current_retriever.invoke, message)
